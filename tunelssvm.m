...
% Uncomment line 222 - 404 and Comment line 407 for applying local search
...
elseif strcmp(model.kernel_type,'RBF_kernel') || strcmp(model.kernel_type,'sinc_kernel') || strcmp(model.kernel_type,'RBF4_kernel')
    if ~strcmp(model.weights,'wmyriad') && ~strcmp(model.weights,'whuber')
        [par,fval] = csa(rand(2,30),@(x)simanncostfun2(x,model,costfun,costargs));
    else
        [par,fval] = csa(rand(3,30),@(x)simanncostfun2(x,model,costfun,costargs));
