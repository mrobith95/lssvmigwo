function [model,cost,O3] = tunelssvmigwo(model, varargin)
...
% WARNING! Local search has taken down!  
% Uncomment line 221 - 403 and Comment line 406 for applying local search
...
if strcmp(model.kernel_type,'lin_kernel'),
    if ~strcmp(model.weights,'wmyriad') && ~strcmp(model.weights,'whuber')
        [par,fval] = igwo(rand(1,5),@(x)simanncostfun1(x,model,costfun,costargs));
    else
        [par,fval] = igwo(rand(2,5),@(x)simanncostfun1(x,model,costfun,costargs));
...
elseif strcmp(model.kernel_type,'RBF_kernel') || strcmp(model.kernel_type,'sinc_kernel') || strcmp(model.kernel_type,'RBF4_kernel')
    if ~strcmp(model.weights,'wmyriad') && ~strcmp(model.weights,'whuber')
        [par,fval] = igwo(rand(2,30),@(x)simanncostfun2(x,model,costfun,costargs));
    else
        [par,fval] = igwo(rand(3,30),@(x)simanncostfun2(x,model,costfun,costargs));
        model.delta = exp(par(3));
    end
    model = changelssvm(changelssvm(model,'gam',exp(par(1))),'kernel_pars',exp(par(2)));
    
    fprintf('\n')
    disp([' 1. Chaos Grey Wolf Optimizer results  :  [gam]         ' num2str(model.gam)]);
...
elseif strcmp(model.kernel_type,'poly_kernel'),
    ...
    warning on
    %[par,fval] = igwo(rand(3,5),@(x)simanncostfun3(x,model,costfun,costargs));
...
elseif strcmp(model.kernel_type,'wav_kernel'),
    if ~strcmp(model.weights,'wmyriad') && ~strcmp(model.weights,'whuber')
        [par,fval] = igwo(rand(4,5),@(x)simanncostfun4(x,model,costfun,costargs));
    else
        [par,fval] = igwo(rand(5,5),@(x)simanncostfun4(x,model,costfun,costargs));
...
% Comment this after uncommenting Local Search function
cost = fval;
